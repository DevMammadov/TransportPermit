...
import { InfoConfirmVM } from "@/ui/modules/SupplyChain/InvalidBlank/newInvalidBlanks/steps/info-confirm/InfoConfirmVM";
...
const InfoConfirmPage = () => {
  const { labelsForInfo, prevStep, control, onSubmit, sendingLoading } =
    InfoConfirmVM();
    ...
    >
      <div className="border border-gray-200 rounded-2xl p-6 text-lg text-slate-800 font-semibold">
        <div className="flex items-center gap-3 justify-between">
          <h3>Kod: {labelsForInfo?.permit?.code} </h3>
          <p className="font-normal text-sm text-slate-700">
            Yaradılma tarixi: - {dayjs().format("DD.MM.YYYY")}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 text-base text-slate-800">
          ...
          <ListItem label="Ölkə" value={labelsForInfo?.permit?.country.value} />
          <ListItem
            label="İcazənin tipi"
            value={labelsForInfo?.permit?.permitType.value}
          />
          <ListItem
            label="İcazənin növü"
            value={labelsForInfo?.permit?.permitCategory.name}
          />
          <ListItem
            label="İcazə blanklarının defisitliyi"
            value={labelsForInfo?.permit?.deficiencyLevelType.value}
          />

...
import { PointTransferVM } from "@/ui/modules/SupplyChain/PointTransfer/PointTransferVM";
...
const PointTransferPage = () => {
  ...
  if (!pointTransferList?.items?.length && !isFiltered(filters)) {
    return (
      <NoResult
        title="Daxil edilmiş kvota yoxdur"
        to={(to) =>
          `${to.pointTransfer.baseUrl}/add/${EPointTransferSteps.QUOTA_INFO}`
        }
  ...
  return (
    <div className="wrapper">
      ...
      <DataTable
        columns={columns}
        data={pointTransferList?.items}
        count={pointTransferList?.totalCount}
        title="Məxaric"
        currentPage={page}
